...
- `VPN_IPSEC_LOCAL_ID`：云主机经公网 EIP/NAT 暴露时，建议显式设置为客户端实际连接的公网 IP 或域名
...
```bash
ssh -L 18080:127.0.0.1:8080 root@119.28.114.185
```
...
3. `macOS` 可从账号详情页下载安装包，解压后双击 `.command`，再在系统描述文件界面确认安装；账号密码、共享密钥和所有 L2TP 接入点会自动预填。
4. 在 `Windows / iPhone / Android 12 以下` 使用原生 `L2TP/IPSec PSK` 接入。
5. 在 `Android 12+` 使用 UniConnect 连接 `openconnect-ssl` 接入点，账号密码沿用后台 VPN 账号。
6. 使用后台导出的 JSON 配置校验网关列表。
7. 连接后观察后台中的会话、流量和异常事件。

macOS 安装包包含账号明文密码和 `VPN_SHARED_PSK`，只应通过受信任渠道分发，并在安装完成后删除。
